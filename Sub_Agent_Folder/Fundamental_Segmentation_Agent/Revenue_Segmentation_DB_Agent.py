        ...
        try:
            ...
            if existing_data:
                ...
                if next_earnings_date:
                    ...
                    # No earnings date in metadata - use 24-hour fallback logic
                    logging.info(f"⚠️ No earnings date found in metadata for {ticker}, using 24-hour fallback logic")
                    
                    # Check stored_at timestamp for 24-hour freshness
                    ...
                    if stored_at:
                        try:
                            ...
                            if hours_since_update < 24:  # 24-hour threshold
                                logging.info(f"✅ Data is fresh for {ticker} ({hours_since_update:.1f} hours old, 24-hour fallback)")
                                return existing_data
                            else:
                                logging.info(f"🔄 Data is stale for {ticker} ({hours_since_update:.1f} hours old, 24-hour fallback), downloading fresh data")
                                success = await self.download_and_store_ticker(ticker, collection_name)
                                
                                if success:
                                    return await self.get_revenue_segmentation_data(ticker, collection_name)
                                else:
                                    logging.error(f"❌ Failed to download fresh data for {ticker} after 24-hour staleness check")
                                    return existing_data
                        except Exception as e:
                            logging.warning(f"⚠️ Could not parse stored_at timestamp for {ticker}: {e}")
                            logging.info(f"✅ Using existing data for {ticker} (timestamp parsing failed)")
                            return existing_data
                    else:
                        # No stored_at timestamp - download fresh data
                        logging.info(f"🔄 No stored_at timestamp found for {ticker}, downloading fresh data")
                        success = await self.download_and_store_ticker(ticker, collection_name)
                        
                        if success:
                            return await self.get_revenue_segmentation_data(ticker, collection_name)
                        else:
                            logging.error(f"❌ Failed to download fresh data for {ticker} (no timestamp)")
                            return existing_data
