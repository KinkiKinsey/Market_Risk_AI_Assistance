...
from config import FMP_API_KEY, TAVILY_API_KEY as Tavily_API_KEY, FMP_STABLE_URL
...
class RevenueSegmentationStorageAgent:
    ...
    def search_ticker_cost_structure(self, ticker: str) -> dict:
        """Search for ticker cost structure information using Tavily (MIMIC of search_ticker_product_revenue)."""
        query = f"What is stock ticker: {ticker} cost structure, COGS breakdown, operating expenses, R&D costs, I want any source mention how it cost breakdown from financial statement or earning report announcement from latest"
        response = self.tavily_client.search(
            query=query,
            topic="general",
            search_depth="advanced",
    ...
    def search_ticker_suppliers(self, ticker: str) -> dict:
        """Search for ticker supplier information using Tavily (MIMIC of search_ticker_product_revenue)."""
        query = f"What is stock ticker: {ticker} main suppliers, supply chain, supplier dependencies, I want any source mention supplier information from financial statement or earning report announcement from latest"
        response = self.tavily_client.search(
            query=query,
            topic="general",
            search_depth="advanced",
    ...
    def revenue_segmentation_tool(self, ticker, max_years_back=5):
        ...
        for y in range(current_year, current_year - max_years_back, -1):
            ...
            for q in quarters_to_try:
                print(f"🔎 Trying: {y} {q}")
                url = f"{FMP_STABLE_URL}/financial-reports-json?symbol={TICKER}&year={y}&period={q}&apikey={FMP_API_KEY}"
            ...
            print(f"🔎 Trying: {y} FY")
            url = f"{FMP_STABLE_URL}/financial-reports-json?symbol={TICKER}&year={y}&period=FY&apikey={FMP_API_KEY}"
            data = self.get_financial_statement(url)
            attempts += 1
            if data and isinstance(data, dict) and data.get("symbol"):
    ...
    def _is_fmp_data(self, raw_data: dict) -> bool:
        """Check if data is from FMP API (comprehensive 10-K style) vs Tavily (focused search)."""
        if not isinstance(raw_data, dict):
            return False
        
        # FMP data has specific keys like 'symbol', 'period', 'year', and many financial statement sections
        # Tavily data has 'answer' and 'sources_content' keys
        has_fmp_keys = any(key in raw_data for key in ['symbol', 'period', 'year'])
        has_tavily_keys = any(key in raw_data for key in ['answer', 'sources_content'])
        
        # FMP data typically has 20+ keys (financial statement sections)
        # Tavily data has 2-3 keys (answer, sources_content)
        key_count = len(raw_data)
        
        # If it has FMP keys and many sections, it's FMP data
        if has_fmp_keys and key_count > 10:
            return True
        
        # If it has Tavily keys, it's Tavily data
        if has_tavily_keys:
            return False
        
        # Fallback: if it has many keys like FMP financial statements, assume FMP
        return key_count > 15
    
    def _extract_revenue_sections(self, raw_data: dict) -> str:
        """Extract only revenue-relevant sections from financial statement to reduce token count.
        
        Logic:
        - If data is from FMP API (comprehensive): Extract only revenue sections (too large otherwise)
        - If data is from Tavily API (focused): Use all data (already filtered and relevant)
        """
        if not isinstance(raw_data, dict):
            return str(raw_data)
        
        # Check if data source is FMP or Tavily
        is_fmp = self._is_fmp_data(raw_data)
        
        if not is_fmp:
            # Tavily data: Use all data (already filtered and relevant)
            print("📝 Using full Tavily data (already focused)")
            result = json.dumps(raw_data, indent=2)
            # Still apply a safety limit, but much higher (500k chars = ~125k tokens)
            if len(result) > 500000:
                result = result[:500000] + "\n... [truncated for safety]"
            return result
        
        # FMP data: Extract only revenue-relevant sections (comprehensive data, needs filtering)
        print("📝 Extracting revenue sections from FMP data (comprehensive)")
        revenue_keywords = ['revenue', 'segment', 'geographic', 'operating', 'business']
        ...
        # If no revenue sections found, include first 3 sections (usually contain important data)
        if not extracted:
            extracted = dict(list(raw_data.items())[:3])
        
        # Convert to string, but limit size to ~100k characters (~25k tokens)
        result = json.dumps(extracted, indent=2)
        if len(result) > 100000:
            # Truncate to first 100k characters
    ...
    def analyze_financial_statement(self, financial_text: str) -> dict:
        ...
        # Ensure financial_text is a string and limit size
        if isinstance(financial_text, dict):
            financial_text = json.dumps(financial_text, indent=2)
        if len(financial_text) > 100000:
            financial_text = financial_text[:100000] + "\n... [truncated for token limit]"
...
"""
    ...
    def analyze_cost_and_supply(self, cost_text: str, supplier_text: str) -> dict:
        """Analyze cost structure and suppliers using LLM (MIMIC of analyze_financial_statement)."""
        print(f"Calling {self.llm_agent.default_provider} to process the cost and supplier information")
        
        prompt = f"""
You are a financial analyst AI. Given the following information about costs and suppliers, extract and structure the company's cost/supply chain into JSON format. Focus on:

COST STRUCTURE:
1. What are the major cost categories? (COGS, R&D, SG&A, etc.)
2. What percentage of revenue each cost represents?
3. Cost Amount
4. Trend (increasing/decreasing/stable)

SUPPLIER INFORMATION:
1. Who are the main suppliers?
2. What do they supply? (components, materials, services)
3. Estimated percentage of COGS
4. Geographic location
5. Dependency risk level
...
{{
  "cost_segments": [
    {{
      "category": "Cost Category Name" (e.g., COGS, R&D, SG&A) (if not read from text, use 'Not specified'),
      "amount": "xxx" (if not read from text, state "Guess $xxx"),
      "percentage_of_revenue": "xx%" (if not read from text, state "Guess xx%"),
      "trend": "increasing/decreasing/stable" (if not read from text, use 'Not specified'),
      "details": "Any additional details about this cost category"
    }}
  ],
  "supplier_segments": [
    {{
      "supplier_name": "Supplier Company Name" (if not read from text, use 'Not specified'),
      "supplies": "What they supply" (if not read from text, use 'Not specified'),
      "estimated_percentage_of_cogs": "xx%" (if not read from text, state "Guess xx%"),
      "geographic_location": "Country/Region" (if not read from text, use 'Not specified'),
      "dependency_risk": "High/Medium/Low" (if not read from text, state "Guess based on context")
    }}
  ]
}}

Here is the cost structure information:
{cost_text}

Here is the supplier information:
{supplier_text}

You need to make very very detailed segment to the cost and supplier information, and make sure the segment is correct.
You are an API call, hence do not return any text outside of the JSON structure, because the user will parse the JSON response.
"""
        
        response = self.llm_agent.call_llm(prompt, system_message="You are a financial report analyst as API agent")
    ...
    def get_upcoming_earnings(self, ticker):
        ...
        earning_url = f"{FMP_STABLE_URL}/earning_calendar?from={FROM_DATE}&to={TO_DATE}&apikey={FMP_API_KEY}"
...
"""
    ...
    async def process_ticker(self, ticker: str) -> dict:
        ...
        NOW INCLUDES: Cost structure and supplier segmentation (NEW ADDITION).
        
        Args:
            ticker (str): Stock ticker symbol
            
        Returns:
            Dict: Complete analysis output with revenue segmentation, cost/supply segmentation, and metadata
        """
        try:
            print(f"🔍 Processing ticker: {ticker}")
            
            # EXISTING: Get raw financial data for revenue
            ...
            # EXISTING: Process with LLM for revenue segmentation
            # Extract only revenue-relevant sections to avoid context limit
            revenue_sections = self._extract_revenue_sections(raw_source_data)
            llm_process_data = self.analyze_financial_statement(revenue_sections)
            if not llm_process_data:
                return {"error": f"Failed to process data with LLM for {ticker}"}
            
            # NEW: Get cost structure data
            print(f"📊 Getting cost structure data for {ticker}...")
            cost_data = self.search_ticker_cost_structure(ticker)
            
            # NEW: Get supplier data
            print(f"🏭 Getting supplier data for {ticker}...")
            supplier_data = self.search_ticker_suppliers(ticker)
            
            # NEW: Analyze cost and supply with LLM
            cost_supply_analysis = {}
            if cost_data or supplier_data:
                print(f"🤖 Analyzing cost/supply data with LLM...")
                cost_text = str(cost_data) if cost_data else "No cost data available"
                supplier_text = str(supplier_data) if supplier_data else "No supplier data available"
                cost_supply_analysis = self.analyze_cost_and_supply(cost_text, supplier_text)
            else:
                print(f"⚠️ No cost/supply data found for {ticker}")
                cost_supply_analysis = {
                    "cost_segments": [],
                    "supplier_segments": []
                }
            
            # EXISTING: Get upcoming earnings (for update timing)
            upcoming_earnings = self.get_upcoming_earnings(ticker)
            
            # Create output with EXTENDED structure (added cost_supply_segmentation)
            output = {
                "ticker": ticker.upper(),
                "stored_at": datetime.now().isoformat(),
                "revenue_segmentation": llm_process_data,  # EXISTING
                "cost_supply_segmentation": cost_supply_analysis,  # NEW
                "metadata": {
                    ...
                    "revenue_segment_count": len(llm_process_data.get('business_segments', [])),
                    "cost_segment_count": len(cost_supply_analysis.get('cost_segments', [])),  # NEW
                    "supplier_segment_count": len(cost_supply_analysis.get('supplier_segments', []))  # NEW
                }
            }
            
            print(f"✅ Successfully processed {ticker}")
            print(f"   - Revenue segments: {output['metadata']['revenue_segment_count']}")
            print(f"   - Cost segments: {output['metadata']['cost_segment_count']}")
            print(f"   - Supplier segments: {output['metadata']['supplier_segment_count']}")
            return output
