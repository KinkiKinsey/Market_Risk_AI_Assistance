...
# Import centralized config (already imported at top)
# FMP_API_KEY and FMP_API_V3_URL are imported from config
...
def quant_market_expectation_read_agent(ticker: str, shared_clients=None):
    """Read stock trend data from Redis using shared_clients or database_connection"""
    redis_key = f"{COLLECTION_NAME}:{ticker.upper()}_trends"

    # Use shared_clients if available, otherwise create connection via database_connection
    if shared_clients:
        client = shared_clients.get_stock_trend_redis()
    else:
    ...
    if is_async:
        import asyncio
        data = asyncio.run(client.get(redis_key))
    else:
...
class QuantImpactStorageAgent:
    ...
    def __init__(self, shared_clients=None, user_id: str = None):
        """
        Initialize the Quant Impact Storage Agent using centralized database_connection module
        
        Args:
            shared_clients: Shared clients for LLM and Redis access (required)
            user_id: User ID for database storage (required - no default)
        """
        ...
        print(f"✅ Quant Impact Storage Agent initialized")
        print(f"📊 Redis: Connected via database_connection")
        print(f"🗄️ Collection: Quant_Impact_INFOS")
    ...
    async def store_8_datasets_as_csv(
        self,
        ticker: str,
        ...
        try:
            ...
            is_async = self._is_async_redis(self.redis_client)
            
            if is_async:
                # Async client - use individual setex calls with await
                for key, csv_data in datasets.items():
                    await self.redis_client.setex(key, 30 * 24 * 60 * 60, csv_data)
            else:
                # Sync client - use pipeline
    ...
    async def retrieve_8_datasets_from_csv(
        self,
        ticker: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if is_async:
                risk_share_data = await self.redis_client.get(f"{base_key}:RISK_SHARE")
            else:
            ...
            for df_name, redis_key in dataframe_keys:
                if is_async:
                    csv_data = await self.redis_client.get(redis_key)
                else:
            ...
            # Retrieve metadata
            if is_async:
                meta_data = await self.redis_client.get(f"{base_key}:META_INFO")
            else:
    ...
    async def process_quant_impact_analysis(
        ...
        try:
            ...
            # Warm up required agents - Market_Expectation_Agent is REQUIRED for step1_get_factors
            ...
            # FMP_API_KEY already imported from config at top of file
            ...
            storage_success = await self.store_8_datasets_as_csv(
                ticker,
                risk_share_index,
                macro_volatility_df,
                micro_volatility_df,
...
async def get_quant_impact_data(ticker: str, language: str = "English") -> Tuple[Dict[str, float], pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, Dict[str, Any]]:
    ...
    """
    try:
        # Get shared_clients and create storage agent with proper initialization
        from shared_clients import shared_clients as sc
        if not sc._initialized:
            await sc.initialize()
        
        # Create storage agent with shared_clients and user_id
        storage_agent = QuantImpactStorageAgent(shared_clients=sc, user_id="default")
        ...
        # Store the 8 datasets as CSV in Redis
        print(f"\n💾 Storing 8 datasets as CSV for {ticker}...")
        storage_success = await storage_agent.store_8_datasets_as_csv(
            ticker,
            risk_share_index,
            macro_volatility_df,
            micro_volatility_df,
