...
class QuantImpactIncrementalUpdate:
    ...
    def __init__(self, shared_clients=None, user_id: str = None):
        ...
        Args:
            shared_clients: Shared clients for LLM operations (required)
    ...
    async def debug_check_datasets_complete(self, ticker: str) -> bool:
        ...
        try:
            ...
            # Use await since we're in async context (called from async method)
            datasets = await self.db_agent.retrieve_datasets(ticker, user_id=self.user_id)
    ...
    async def incremental_update(self, ticker: str, language: str = "English") -> Dict[str, Any]:
        ...
        try:
            ...
            if await self.db_agent.check_data_exists(ticker, user_id=self.user_id):
                print(f"✅ Data exists for {ticker}")
                
                # DEBUG: Check if all 8 datasets are complete
                if await self.debug_check_datasets_complete(ticker):
    ...
    async def _generate_fresh_datasets(self, ticker: str, language: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await self.db_agent.generate_and_store(ticker, user_id=self.user_id)
    ...
    async def _perform_incremental_update(self, ticker: str, language: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                from Source_File.Agent_Folder.Sub_Agent_Folder.Market_Expectation_Agent.Stock_Trend_DB_Agent import DatabaseStorage
...
async def run_incremental_update(ticker: str, language: str = "English", shared_clients=None, user_id: str = "default") -> Dict[str, Any]:
    """
    ...
    Args:
        ticker: Stock ticker symbol
        language: Language for output
        shared_clients: Shared clients pool (required, will initialize if None)
        user_id: User ID for database storage (default: "default")
        
    Returns:
        Update results summary
    """
    # Initialize shared_clients if not provided
    if not shared_clients:
    ...
    agent = QuantImpactIncrementalUpdate(shared_clients=shared_clients, user_id=user_id)
