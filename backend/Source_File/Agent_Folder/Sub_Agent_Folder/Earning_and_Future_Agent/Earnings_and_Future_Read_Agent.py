...
Analyze the earnings and future development data to provide comprehensive insights about {ticker}."""
    ...
    async def check_database_status(self, ticker: str, force_update: bool = False) -> Dict:
        ...
        try:
            ...
            stored_at = earnings_data.get('stored_at')
            if stored_at:
                ...
                if days_since_update < 30 and not force_update:
                    return {
                        "status": "fresh",
                        "message": f"Earnings data is fresh ({days_since_update} days old, threshold: 30 days)",
                    ...
                    return {
                        "status": "stale",
                        "message": f"Earnings data is {days_since_update} days old (threshold: 30 days)",
                        "days_since_update": days_since_update,
    ...
    async def run_earnings_analysis_if_needed(self, ticker: str, force_update: bool = False) -> Dict:
        ...
        Only returns success if data is fresh (< 30 days old).
