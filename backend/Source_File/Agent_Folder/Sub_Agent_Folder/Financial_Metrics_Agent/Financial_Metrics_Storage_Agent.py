...
from config import FMP_API_KEY as fmp_api_key, TAVILY_API_KEY as tavily_api_key, FMP_STABLE_URL
...
def get_latest_quarterly_valuation_metrics(ticker, api_key):
    ...
    if backend_dir not in sys.path:
        sys.path.insert(0, backend_dir)
    # FMP_STABLE_URL already imported at top
    
    url = f"{FMP_STABLE_URL}/key-metrics?symbol={ticker}&period=quarter&limit=1&apikey={api_key}"
...
def get_one_month_year_stock_prices_df(ticker, api_key):
    ...
    url = f"{FMP_STABLE_URL}/historical-price-eod/non-split-adjusted?symbol={ticker}&from={from_date}&to={to_date}&apikey={api_key}"
...
def get_latest_dcf_valuation_with_fallback(ticker, api_key, tavily_api_key):
    ...
    # First try FMP API
    url = f"{FMP_STABLE_URL}/discounted-cash-flow?symbol={ticker}&apikey={api_key}"
...
def get_ticker_sector_info_with_fallback(ticker, fmp_api_key, tavily_api_key):
    ...
    """
    # First try FMP API
    url = f"{FMP_STABLE_URL}/search-exchange-variants?symbol={ticker}&apikey={fmp_api_key}"
