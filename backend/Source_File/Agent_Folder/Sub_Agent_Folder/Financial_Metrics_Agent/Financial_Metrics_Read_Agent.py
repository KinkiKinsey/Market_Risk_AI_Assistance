    ...
    def check_update_status(self, ticker: str) -> Dict:
        ...
        try:
            ...
            if current_date == update_date:
                ...
                # Different day - apply 6:00 PM rule
                days_difference = (current_date - update_date).days
                
                if days_difference == 1:
                    # Yesterday - check if we're past 6:00 PM today
                    six_pm_today = current_time.replace(hour=18, minute=0, second=0, microsecond=0)
                    if current_time >= six_pm_today:
                        # Past 6:00 PM - need update
                        return {
                            "ticker": ticker.upper(),
                            "status": "update_needed",
                            "message": f"Data is from yesterday, past 6:00 PM today - update needed",
                            "last_update": last_update,
                            "current_time": current_time.isoformat(),
                        ...
                        # Before 6:00 PM - data is still fresh
                        time_until_update = six_pm_today - current_time
                        return {
                            "ticker": ticker.upper(),
                            "status": "fresh",
                            "message": f"Data is from yesterday, before 6:00 PM today - still fresh",
                            "last_update": last_update,
                            "current_time": current_time.isoformat(),
                            "update_date": update_date.isoformat(),
                            "current_date": current_date.isoformat(),
                    ...
                    # More than 1 day old - definitely need update
                    return {
                        "ticker": ticker.upper(),
                        "status": "update_needed",
