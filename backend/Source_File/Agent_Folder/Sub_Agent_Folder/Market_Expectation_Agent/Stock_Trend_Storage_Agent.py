...
from config import FMP_API_KEY, FMP_STABLE_URL, DEEPSEEK_API_KEY, DEEPSEEK_BASE_URL

async def deepseek_api_call(prompt, base_url=None, model="deepseek-chat"):
    if base_url is None:
...
def get_stock_news_fmp(api_key: str, ticker: str, start_date: str, end_date: str, page: int = 0):
    ...
    # --- Stock-specific news ---
    stock_news_url = f"{FMP_STABLE_URL}/news/stock"
    stock_params = {
        "symbols": ticker,
        "from": start_date,
        "to": end_date,
    ...
    # --- General news ---
    general_news_url = f"{FMP_STABLE_URL}/news/general-latest"
    general_params = {
        "from": start_date,
        "to": end_date,
        "page": 1,
    ...
    # --- Press releases ---
    # Import FMP_API_V3_URL if not already imported
    try:
        from config import FMP_API_V3_URL
    except:
        FMP_API_V3_URL = "https://financialmodelingprep.com/api/v3"
    press_releases_url = f"{FMP_API_V3_URL}/press-releases/{ticker}"
    press_params = {
        "page": page,
        "apikey": api_key,
    }
...
async def analyze_stock_trends(ticker: str, force_update: bool = False, use_multiprocessing: bool = True):
    ...
    try:
        ...
        if not time_list:
            ...
            if not time_list:
                print("❌ No trend segments found - insufficient data for analysis")
                error_metadata = create_metadata(ticker, current_time.isoformat())
                error_metadata["error"] = "No trend segments found - insufficient price data for analysis"
                error_metadata["analysis_period_days"] = (datetime.today() - datetime.strptime(start_date, "%Y-%m-%d")).days
                return {}, {}, error_metadata
