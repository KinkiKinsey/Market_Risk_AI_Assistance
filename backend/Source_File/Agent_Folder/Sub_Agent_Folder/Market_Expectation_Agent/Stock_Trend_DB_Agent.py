...
class RedisDatabaseStorage:
    ...
    async def store_stock_trend_data(self, ticker: str, current_json: Dict, historical_json: Dict, metadata: Dict, collection_name: str = "Stock_Trend_INFOS") -> bool:
        """
    ...
    async def get_stock_trend_data(self, ticker: str, collection_name: str = "Stock_Trend_INFOS") -> Optional[Dict]:
        """
        ...
        try:
            ...
            redis_key = f"{collection_name}:{ticker.upper()}_trends"
            # Always use await for aioredis client (from shared_clients)
            # Check if it's an aioredis client (has __class__.__name__ == 'Redis')
            if hasattr(self.client, '__class__') and 'aioredis' in str(type(self.client)):
                # Async client - use await
                data_str = await self.client.get(redis_key)
            elif hasattr(self.client, 'get'):
                # Sync redis client
                data_str = self.client.get(redis_key)
            else:
                logging.error(f"❌ Unknown Redis client type: {type(self.client)}")
                return None
    ...
    def list_stock_tickers(self, collection_name: str = "Stock_Trend_INFOS") -> List[str]:
        """
    ...
    async def update_if_stale_with_lock(self, ticker: str, collection_name: str = "Stock_Trend_INFOS", force_update: bool = False) -> str:
        """
        ...
        try:
            ...
            existing_data = await self.get_stock_trend_data(ticker, collection_name)
            ...
            if existing_data and not force_update:
                ...
                if stored_at:
                    try:
                        ...
                        if hours_since_update < 24:  # Market Expectation: 24 hours
            ...
            if is_async:
                lock_acquired = await self.client.set(lock_key, "locked", ex=300, nx=True)
            else:
                lock_acquired = self.client.set(lock_key, "locked", ex=300, nx=True)
            ...
            if lock_acquired:
                ...
                for _ in range(max_wait_time // wait_interval):
                    if is_async:
                        ...
                        lock_exists = self.client.exists(lock_key)
...
class DatabaseStorage:
    ...
    def store_stock_trend_data(self, ticker: str, current_json: Dict, historical_json: Dict, metadata: Dict, collection_name: str = "Stock_Trend_INFOS") -> bool:
    ...
    async def get_stock_trend_data(self, ticker: str, collection_name: str = "Stock_Trend_INFOS") -> Optional[Dict]:
        ...
        return await self.storage.get_stock_trend_data(ticker, collection_name)
    
    async def update_if_stale_with_lock(self, ticker: str, collection_name: str = "Stock_Trend_INFOS", force_update: bool = False) -> str:
        """Update shared data with locking to prevent duplicate expensive API calls."""
        if hasattr(self.storage, 'update_if_stale_with_lock'):
            return await self.storage.update_if_stale_with_lock(ticker, collection_name, force_update)
        else:
    ...
    def get_collection_stats(self, collection_name: str = "Stock_Trend_INFOS") -> Dict:
    ...
    async def download_and_store_ticker(self, ticker: str, collection_name: str = "Stock_Trend_INFOS", force_update: bool = False) -> bool:
        """
    ...
    async def store_stock_trend_files(self, ticker: str, current_file: str, historical_file: str, metadata_file: str, collection_name: str = "Stock_Trend_INFOS") -> bool:
        """
...
def setup_logging(level: str = "INFO"):
    ...
    # Create handlers (file logging disabled)
    ...
    root_logger.setLevel(logging.WARNING)  # Changed to WARNING to reduce output
    # root_logger.addHandler(file_handler)  # File handler disabled
...
def test_update_locking():
    ...
    def simulate_user_request(user_id, ticker):
        ...
        result = storage.update_if_stale_with_lock(ticker, force_update=False)
