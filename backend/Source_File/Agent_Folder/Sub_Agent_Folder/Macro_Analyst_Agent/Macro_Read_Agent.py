...
class MacroReadAgent:
    ...
    async def read_macro_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check data freshness when accessing database
            ...
            if freshness_status['needs_update']:
                ...
                if not update_success:
                    return {'error': 'Failed to update macro data'}
                
                print("✅ Update completed. Now reading fresh data...")
    ...
    async def process_user_query(self, user_query: str) -> str:
        ...
        try:
            ...
            # Read macro data (freshness check happens inside read_macro_data)
        ...
        try:
            if hasattr(self, 'macro_db_agent') and self.macro_db_agent:
                ...
                if hasattr(self.macro_db_agent, 'close'):
                    await self.macro_db_agent.close()
...
Always provide both sections with this exact structure for every query."""
    ...
    def _generate_fallback_response(self, query: str, analysis: str, metadata: Dict, indicators: List[str]) -> str:
        """
        Generate error response when LLM fails - no mock data
        """
        return f"""❌ **Analysis Error for: {query}**

**ERROR:** LLM analysis service unavailable
**EVIDENCE:** Cannot process intelligent queries  
**RESULT:** Analysis cannot be completed

**Available Data:**
- {len(indicators)} economic indicators
- Data range: {metadata.get('data_range', {}).get('start_date', 'Unknown')} to {metadata.get('data_range', {}).get('end_date', 'Unknown')}

**Action Required:** Ensure LLM service is available for analysis."""
