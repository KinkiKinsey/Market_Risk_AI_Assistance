...
# Custom imports - Add backend to path for relative imports
import sys
import os

# Get backend directory (3 levels up from Agent_Folder/Mid_Agent_Folder/Hedge_Fund_Brain.py)
backend_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
if backend_dir not in sys.path:
    sys.path.insert(0, backend_dir)

from update_pool import update_pool
from Source_File.Agent_Folder.Mid_Agent_Folder.Data_Retrieval import (
    get_earnings_data,
    get_sector_data,
...
# Import centralized config (backend_dir already set above)
...
# ==============================================================
# Redis Functions - Using Unified Database Connection
# ==============================================================

def _is_async_redis(client) -> bool:
    """Check if Redis client is async (aioredis) or sync (redis)"""
    return hasattr(client, '__class__') and 'aioredis' in str(type(client))

async def check_freshness(ticker: str, shared_clients=None) -> bool:
    ...
    try:
        ...
        db_storage = RedisDatabaseStorage(db_type="frontend", shared_clients=shared_clients)
        client = db_storage.redis_client
        
        brain_key = f"Hedge_Fund_Brain:{ticker.upper()}:brain"
        
        # Handle both async and sync clients
        is_async = _is_async_redis(client)
        if is_async:
            brain_data = await client.get(brain_key)
        else:
...
async def retrieve_from_redis(ticker: str, shared_clients=None) -> Tuple[Dict, List]:
    ...
    try:
        ...
        if is_async:
            brain_data = await client.get(brain_key)
            alpha_data = await client.get(alpha_key)
        else:
            brain_data = client.get(brain_key)
            alpha_data = client.get(alpha_key)
...
async def store_to_redis(ticker: str, brain: Dict, alpha: List, shared_clients=None):
    ...
    try:
        ...
        is_async = _is_async_redis(client)
        if is_async:
            await client.set(brain_key, json.dumps(brain_package))
            await client.set(alpha_key, json.dumps(alpha_package))
            await client.expire(brain_key, 86400 * 7)  # 7 days
            await client.expire(alpha_key, 86400 * 7)
        else:
            client.set(brain_key, json.dumps(brain_package))
            client.set(alpha_key, json.dumps(alpha_package))
            client.expire(brain_key, 86400 * 7)
            client.expire(alpha_key, 86400 * 7)
        
    except Exception as e:
        print(f"⚠️ Redis storage failed: {e}")


async def generate_new_brain(ticker: str, shared_clients=None) -> Tuple[Dict, List]:
    ...
    # Store to Redis
    await store_to_redis(ticker, brain, alpha, shared_clients)
...
async def hedgefundbrain(ticker: str, shared_clients=None) -> Tuple[Dict, List]:
    """
    ...
    Args:
        ...
        shared_clients: Optional SharedClientPool for connection pooling (recommended)
        
    Returns:
        Tuple[Dict, List]: (brain, alpha_insights)
        
    Example with shared_clients (recommended):
        from shared_clients import shared_clients
        await shared_clients.initialize()
        brain, alpha = await hedgefundbrain("TGT", shared_clients=shared_clients)
        
    Example without shared_clients (fallback):
        brain, alpha = await hedgefundbrain("TGT", shared_clients=None)
    ...
    is_fresh = await check_freshness(ticker, shared_clients)
    
    if is_fresh:
        # Data < 7 days old, retrieve from Redis
        print(f"✅ Fresh data found for {ticker} (< 7 days old)")
        brain, alpha = await retrieve_from_redis(ticker, shared_clients)
    ...
    brain, alpha = await generate_new_brain(ticker, shared_clients)
