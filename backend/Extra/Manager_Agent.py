...
class ManagerAgent:
    ...
    async def process_manager_query(self, user_query: str, ticker: str, language: str = "English") -> Manager_Agent_Result:
        ...
        3. FINANCIAL METRICS ANALYST AGENT:
        ...
        ROUTING RULES:
        1. **CALL EXACTLY 2 AGENTS** - Choose the 2 most relevant agents for this query out of 5 available agents
        ...
        - Query: "Tariff impact on LULU"
          ...
          → Financial Metrics: "Analyze how tariffs affect LULU's cost structure and margins"
        ...
        - Query: "Tesla market share in EV sector"
          ...
          → Financial Metrics: "Analyze Tesla's financial metrics relative to EV sector peers"
        ...
        - Choose the 2 most relevant agents for this specific query out of 5 available agents
        - Keep queries short and focused
        - Use "N/A" for agents you don't call
        - Set decision = 1 for exactly 2 agents, decision = 0 for the other 3 agents
    ...
    def create_agent_calling_form(self, result: Manager_Agent_Result) -> Dict[str, Any]:
        ...
        decision_list = [Decision_call_revenue_segmentation, 
        ...
        if agents_called != 2:
            ...
            if agents_called > 2:
                ...
                result.Decision_call_revenue_segmentation = decision_list[0]
                result.Decision_call_macro_analyst = decision_list[1]
                result.Decision_call_financial_metrics_analyst = decision_list[2]
                result.Decision_call_earnings_and_future = decision_list[3]
                result.Decision_call_sector_analyst = decision_list[4]
            elif agents_called < 2:
                print("🔧 Auto-correcting: Adding Revenue Segmentation Agent to reach 2 agents")
                ...
                if result.Decision_call_macro_analyst == 0:
                    result.Decision_call_macro_analyst = 1
                    result.query_for_macro_analyst = "Analyze macroeconomic factors"
        ...
        Decision_List = [Decision_call_revenue_segmentation, Decision_call_macro_analyst, Decision_call_financial_metrics, Decision_call_earnings_and_future, Decision_call_sector_analyst]
        Agent_List = ["Revenue_Segmentation_Agent", "Macro_Analyst_Agent", "Financial_Metrics_Agent", "Earnings_and_Future_Agent", "Sector_Analyst_Agent"]
        Query_List = [Revenue_Segmentation_Query, Macro_Query, Financial_Metrics_Query, Earnings_and_Future_Query, Sector_Analyst_Query]
    ...
    async def call_agents_dynamically(self, Decision_List: List[int], Agent_List: List[str], Query_List: List[str], ticker: str, shared_clients=None):
        ...
        async def run_revenue(q):
            ...
            from Sub_Agent_Folder.Fundamental_Segmentation_Agent import FundamentalSegmentationAgent
            if shared_clients:
        ...
        async def run_macro(q):
            from Sub_Agent_Folder.Macro_Analyst_Agent import MacroAnalystAgent
            import time
            if shared_clients:
                a = MacroAnalystAgent(
                    shared_clients=shared_clients,
        ...
        async def run_financial(q):
            from Sub_Agent_Folder.Financial_Metrics_Agent.Financial_Metrics_Analyst_Agent import FinancialMetricsAnalystAgent
            import time
            if shared_clients:
        ...
        async def run_earnings(q):
            from Sub_Agent_Folder.Earning_and_Future_Agent.Earnings_and_Future_Read_Agent import EarningsAndFutureReadAgent
            if shared_clients:
        ...
        async def run_sector(q):
            ...
            from Sub_Agent_Folder.Sector_Analyst_Agent import SectorAnalystAgent
            if shared_clients:
        ...
        for decision, name, q in zip(Decision_List, Agent_List, Query_List):
            ...
            if name == "Revenue_Segmentation_Agent":
