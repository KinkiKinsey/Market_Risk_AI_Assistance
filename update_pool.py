...
async def update_pool(ticker: str) -> Dict[str, Any]:
    """
    SMART Data Pool Function - Directly checks each agent's database for update information
    
    Logic:
    1. Directly access each agent's Redis database
    ...
    Returns:
        Dict[str, Any]: Results from all sub-agents with execution details
    """
    ...
    try:
        ...
        print("🔍 Checking each agent's database for update status...")
        
        # Check each agent's update status
        update_status = await check_all_agents_update_status(ticker, frontend_redis, stock_trend_redis)
        ...
        if not agents_to_update:
            print("\n✅ All agents have fresh data - no updates needed!")
            return {
                "status": "all_fresh",
                "execution_time": time.time() - start_time,
                "message": "All agents have fresh data",
                "results": {}
            }
        ...
        # Import agents
        from Market_Expectation_Agent.Stock_Trend_Read_Agent import StockTrendAnalystAgent
        from Fundamental_Segmentation_Agent.Revenue_Segmentation_Read_Agent import RevenueSegmentationAnalystAgent
        from Financial_Metrics_Agent.Financial_Metrics_Read_Agent import FinancialMetricsReadAgent
        from Macro_Analyst_Agent.Macro_Read_Agent import MacroReadAgent
        from Earning_and_Future_Agent.Earnings_and_Future_Read_Agent import EarningsAndFutureReadAgent
        from Sector_Analyst_Agent.Sector_Analyst_Read_Agent import SectorAnalystReadAgent
        ...
        if "market_expectation" in agents_to_update:
            async def update_market():
                ...
                result = await agent.process_natural_query(f"Get latest trend data for {ticker}", ticker, force_update=False)
                print(f"✅ Market Expectation updated for {ticker}")
                return "stock_trend", result
            tasks.append(asyncio.create_task(update_market()))
        ...
        if "macro_analysis" in agents_to_update:
            async def update_macro():
                ...
                result = await agent.process_user_query(f"Get latest macro factors affecting {ticker}")
        ...
        return {
            ...
            "agents_checked": len(update_status),
...
async def check_all_agents_update_status(ticker: str, frontend_redis, stock_trend_redis) -> Dict[str, Dict]:
    """
    Check update status for all agents using EXACT same logic as Read Agents
    
    Returns:
        Dict: Update status for each agent
    """
    update_status = {}
    ...
    # 2. Financial Metrics Agent - EXACT COPY from Financial_Metrics_Read_Agent.py lines 368-477
    try:
        ...
        if data:
            ...
            if not last_update:
                ...
                # EXACT COPY of 6 PM rule from Read Agent
                current_time = datetime.now()
                ...
                if current_date == update_date:
                    ...
                    if current_time >= six_pm_today:
                        ...
                        if last_update_hour < 18:
                            ...
                            update_status["financial_metrics"] = {
                                ...
                                "reason": f"Past 6:00 PM on {current_date}, first update after 6 PM needed"
                            }
                        else:
                            ...
                            update_status["financial_metrics"] = {
                                "needs_update": False,
                                "last_update": last_update,
                                "reason": f"Already updated after 6:00 PM today, data is fresh"
                            }
                    else:
                        ...
                        update_status["financial_metrics"] = {
                            "needs_update": False,
                            "last_update": last_update,
                            "reason": f"Data is fresh, next update at 6:00 PM on {current_date}"
                        }
                else:
                    # Different day
                    ...
                    update_status["financial_metrics"] = {
                        "needs_update": True,
                        "last_update": last_update,
                        "reason": f"Data is from different day, {days_difference} day(s) old"
                    }
        else:
            update_status["financial_metrics"] = {
                "needs_update": True,
    ...
    # 3. Earnings and Future Agent - EXACT COPY from Earnings_and_Future_Read_Agent.py lines 314-384
    try:
        # Direct Redis access using the same logic as DB Agent
            ...
            if stored_at:
                ...
                if hours_since_update < 24:  # 24-hour threshold from Read Agent
                    update_status["earnings_future"] = {
                        ...
                        "last_update": stored_at,
                        "hours_since_update": hours_since_update,
                        "reason": f"Earnings data is fresh (updated {hours_since_update:.1f} hours ago)"
                    }
                else:
                    update_status["earnings_future"] = {
                        "needs_update": True,
                        "last_update": stored_at,
                        "hours_since_update": hours_since_update,
                        "reason": f"Earnings data is {hours_since_update:.1f} hours old"
                    }
            else:
                update_status["earnings_future"] = {
                    "needs_update": True,
    ...
    try:
        ...
        if stored_data:
            ...
            # Check stored_at timestamp - EXACT field from Read Agent (after our fix)
