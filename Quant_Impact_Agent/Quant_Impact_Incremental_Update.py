...
class QuantImpactIncrementalUpdate:
    ...
    async def incremental_update(self, ticker: str, language: str = "English") -> Dict[str, Any]:
        ...
        try:
            ...
            if self.db_agent.check_data_exists(ticker):
                print(f"✅ Data exists for {ticker}, performing incremental update...")
                return await self._perform_incremental_update(ticker, language)
            else:
                print(f"⚠️ No data found for {ticker}, generating fresh datasets...")
                return await self._generate_fresh_datasets(ticker, language)
    ...
    async def _perform_incremental_update(self, ticker: str, language: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Step 3: Generate new metrics from previous update to today
            print("🔄 Step 2: Generating new metrics...")
            ...
            # Step 4: LLM-based factor mapping and merging
            print("🤖 Step 3: LLM-based factor mapping...")
            ...
            # Step 5: Generate all 8 datasets using Storage Agent pipeline
            print("📊 Step 4: Generating all 8 datasets...")
            all_datasets = await self._generate_all_8_datasets(updated_impact_metrics_df, ticker, language)
            
            # Step 6: Store updated datasets (replace old ones)
            print("💾 Step 5: Storing updated datasets...")
