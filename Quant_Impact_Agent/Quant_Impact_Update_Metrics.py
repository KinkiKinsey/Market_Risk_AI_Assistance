...
def read_stock_trend_data_filtered(ticker: str, previous_update_time: str) -> Dict[str, Any]:
    ...
    for trend_key, trend_data in all_trends.get('historical_trends', {}).items():
        # Get the start date of this trend
        time_data = trend_data.get('time', {})
        start_date_str = time_data.get('start', '')
        
        if start_date_str:
            try:
                start_date = datetime.strptime(start_date_str, '%Y-%m-%d')
                # Only include trends that start on or after the cutoff date
                if start_date >= cutoff_date:
                    filtered_trends[trend_key] = trend_data
            except:
                continue
